...
const get_instance_config = async (instance_slug) => {
  const url = `${root_url(instance_slug)}/static/instances/${instance_slug}.instance.json`
...
const get_geodata = async (instance_slug) => {
  ...
  for (level of levels) {
    const url = `${root_url(instance_slug)}/static/geo/${instance_slug}/spatial_hierarchy/${instance_slug}.${level.name}.geojson`
