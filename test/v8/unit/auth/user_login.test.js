...
test('POST /v8/login with wrong credentials => 401', async t => {
    t.plan(2)
...
test('POST /v8/login with correct credentials => 200', async t => {
    t.plan(4)
    ...
    const res = await request(app).post('/v8/login?country=swz').send(creds)
